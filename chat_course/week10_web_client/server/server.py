...
class ChatServer:
    ...
    def __init__(self, codec, rooms, connections):
        ...
        # 여러 접속(스레드)이 동시에 방·상태를 건드리지 않게 (2주차 with lock 의 그것).
        # RLock = 같은 스레드가 겹쳐 잡아도 안전(재진입).
    ...
    def on_connect(self, conn, nickname):
        with self._lock:
            ...
            count = len(self.connections.all())
    ...
    def on_line(self, session, line):
        try:
            ...
            return
        try:
            with self._lock:                   # 처리 중엔 상태를 한 번에 하나씩만
                # 명령이면 명령 해석기에 위임
            ...
            # 예상 못한 오류가 나도 서버·연결은 살아 있게 (그 한 줄만 버린다)
            print(f"[경고] 메시지 처리 중 오류 무시: {type(e).__name__}: {e} ({session.nickname})")
...
def main():
    ...
    def handle(conn, addr):
        # errors="replace": 깨진 바이트(브라우저·포트스캔·잘못된 클라)가 와도 readline 이
        #                   죽지 않는다. 깨진 줄은 뒤에서 해독 실패로 조용히 버려진다.
        ...
        except Exception:          # OSError 뿐 아니라 어떤 오류든 스레드가 조용히 정리되게
            pass
        finally:
    ...
    try:
        while True:
            try:
                ...
                continue          # 한 번의 accept 실패로 서버 전체가 멈추지 않게
