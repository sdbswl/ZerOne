...
class CommandHandler:
    ...
    def cmd_whisper(self, session, arg):
        ...
        if not session.room:  #귓속말을 하려면 먼저 방에 있어야함
            self.server.send_to(session, SystemMessage("먼저 방에 들어가야 귓속말을 할 수 있습니다."))
            return
    ...
    def cmd_me(self, session, arg):
        if not arg: 
            self.server.send_to(session, SystemMessage("사용법: /me 행동내용"))
            return  
        if not session.room:
            self.server.send_to(session, SystemMessage("먼저 방에 들어가야 합니다. /join 방이름"))
            return
        session.room.post( ##history에 남기고 싶으면 session.room.post()
            SystemMessage(f"{session.nickname} {arg}"),
            self.sever.raw_send
        )
         #전체 접속자 수 보여주는 /count추가
    def cmd_count(self, session, arg):
        total = len(self.server.connections.all())
        self.server.send_to(session, SystemMessage(f"현재 전제 접속자수: {total}명"))